...
async def execute_write(
    ...
    below (unlike the analogous inspect_only path in
    platforms/super_parrain/writer.py): Kraken is currently NO_SAFE_DIFF
    on parrainage-co (already in sync), and inspection needs a real
    authenticated look at the live form regardless of whether a content
    diff currently exists.
    """
    steps: list[str] = []
    circ = live_write_blocked_reason("parrainage-co")
    ...
    async with async_playwright() as pw:
        ...
        try:
            ...
            maybe_trip_from_error(str(exc), platform="parrainage-co")
            return WriteResult(
