#!/usr/bin/env python3
"""Write controle 1Parrainage — plan | canary execute (login/edit/save/reread)."""
...
def main() -> int:
    ...
    plan = build_write_plan("1parrainage", args.program, args.language)
    ...
    if not args.execute:
        ...
        path = OUT / f"write-1parrainage-{args.program}-plan.json"
    ...
    if result.ok and result.post_match:
        ...
        golden_path("1parrainage", args.program, args.language).write_bytes(
            plan.rendered.encode("utf-8")
        )
        mpath = mapping_path("1parrainage", args.program, args.language)
        ...
        evidence = {
            ...
            "source": "controlled_write_1parrainage",
    ...
    try:
        ...
        mark_canary_failed("1parrainage", result.error or "write_failed", program=args.program)
    except Exception:
        pass
    return 1
