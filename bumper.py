...
async def inject_turnstile_token(page, token):
    """Injecte le token Turnstile dans la page."""
    try:
        await page.evaluate(f"""
            (token) => {{
                // Methode 1: champ hidden cf-turnstile-response
                const fields = document.querySelectorAll(
                    'input[name="cf-turnstile-response"], textarea[name="cf-turnstile-response"]'
                );
                fields.forEach(f => f.value = token);

                // Methode 2: callback Turnstile si disponible
                if (window.turnstile && window.turnstile.execute) {{
                    try {{ window.turnstile.execute(); }} catch(e) {{}}
                }}

                // Methode 3: callback direct
                const widget = document.querySelector('[data-sitekey]');
                if (widget && widget.dataset && widget.dataset.callback) {{
                    try {{
                        const cb = window[widget.dataset.callback];
                        if (typeof cb === 'function') cb(token);
                    }} catch(e) {{}}
                }}
            }}
        """, token)
        log.info("  Injection token OK")
        return True
    except Exception as e:
