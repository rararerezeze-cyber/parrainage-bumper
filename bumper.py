...
async def bump_parrainage(page: Page):
    ...
    async def _do():
        # ── Login via cookie "remember me" (bypass Cloudflare, dure des mois) ──
        rm_cookie = os.environ.get("PARRAINAGE_CO_RM_COOKIE", "")
        if not rm_cookie:
            raise RuntimeError("PARRAINAGE_CO_RM_COOKIE manquant dans les secrets")

        # Injecter UNIQUEMENT le cookie remember-me
        # Le serveur crée automatiquement un nouveau PHPSESSID à partir de lui
        ...
        log.info(f"  🍪 Cookie remember-me injecté")
        ...
        if not await verify_login(page, "/login", name):
            raise RuntimeError("Cookie remember-me expiré ou invalide")er-parrain.com | code-parrainage.net | parrainage.co
  Tourne 24h/24 sur GitHub Actions, zéro intervention
=============================================================
"""
...
async def bump_parrainage(page: Page):
    ...
    async def _do():
        ...
        # Injecter uniquement le cookie remember-me (longue durée)
        # Le PHPSESSID est temporaire, parrainageco_rm dure des mois
        rm_cookie = ""
        for part in session_cookies.split(";"):
            part = part.strip()
            if "=" in part:
                cname, cvalue = part.split("=", 1)
                if "rm" in cname.lower() or "remember" in cname.lower():
                    rm_cookie = cvalue.strip()
                    break

        if not rm_cookie:
            # Fallback : prendre le premier cookie disponible
            first = session_cookies.split(";")[0].strip()
            if "=" in first:
                rm_cookie = first.split("=", 1)[1].strip()
        ...
        # Aller sur la page d'accueil — le serveur va créer un nouveau PHPSESSID
        await page.goto(f"{cfg['url']}/account", wait_until="domcontentloaded", timeout=60000)
        await human_sleep(2, 4)
        await page.screenshot(path="debug_parrainage_login.png")

        if not await verify_login(page, "/login", name):
            raise RuntimeError("Cookie remember-me expiré ou invalide")
